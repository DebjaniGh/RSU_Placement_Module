...
def genetic_algorithm(
    ...
    PROBABILITY_LOCAL=0.5,
    MAX_LOCAL_GENERATIONS=15,
    ...
    def bitclimber(child):
        current = child
        for _ in range(MAX_LOCAL_GENERATIONS):
            candidate = mutate(current)
            current = candidate if fitness_function(candidate) >= fitness_function(current) else current
    ...
    for _ in range(GENERATION_COUNT):
        ...
        for i in range(len(new_generation)):
            child = new_generation[i]
            if random.random() < PROBABILITY_LOCAL:
                child = bitclimber(child)
