...
def genetic_algorithm(
    ...
    def randomSolution(lst_of_new_individuals):
        """takes the list of the new individuals created after mutation"""
        # locations = list(range(len(lst_of_new_individuals)))
        # solution = []

        locations = list(lst_of_new_individuals)
        # print(len(locations))
        solution = []

        for _ in range(len(locations)):
            randomLocation = locations[random.randint(0, len(locations) - 1)]
    ...
    def getBestNeighbour(neighbours):
        bestFitnessVal = fitness_function(tuple(neighbours[0]))
        bestNeighbour = neighbours[0]

        for neighbour in neighbours:
            currentFitnessVal = fitness_function(tuple(neighbour))
            if currentFitnessVal > bestFitnessVal:
                bestFitnessVal = currentFitnessVal
                bestNeighbour = neighbour
        return bestNeighbour, bestFitnessVal

    def hillClimbing(new_individuals):
        currentSolution = randomSolution(new_individuals)
        currentFitnesVal = fitness_function(currentSolution)
        neighbours = getNeighbours(currentSolution)
        bestNeighbour, bestNeighbourFitnessVal = getBestNeighbour(neighbours)

        while bestNeighbourFitnessVal > currentFitnesVal:
            currentSolution = bestNeighbour
            currentFitnesVal = bestNeighbourFitnessVal
            neighbours = getNeighbours(currentSolution)
            bestNeighbour, bestNeighbourFitnessVal = getBestNeighbour(neighbours)

        return currentSolution, currentFitnesVal
    ...
    for _ in range(GENERATION_COUNT):
        ...
        for i in range(len(offspring)):
            member_after_LS, val = hillClimbing(offspring[i])
            new_offspring.append(tuple(member_after_LS))

        new_generation.extend(new_offspring)
